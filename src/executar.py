...
Este é o arquivo que o GitHub Actions chama todo dia. Ele faz, em ordem:
  ...
  2. Confere os 13 jogos que tinham sido gerados na véspera
...
from src.coleta import atualizar, carregar_base
...
def executar(simular: bool = False, publicar_redes: bool = True) -> int:
    ...
    # 2. conferência dos jogos da véspera
    passo(2, "Conferindo os jogos já gerados",
          lambda: desempenho.conferir_pendentes(verboso=True), obrigatorio=False)
    ...
    def gerar_jogos():
        ...
        return r
    passo(3, f"Gerando os 13 jogos do concurso {proximo}", gerar_jogos)
    ...
    def gerar_pecas():
        ...
        return r
    passo(4, "Gerando as peças visuais", gerar_pecas)
    ...
    def gerar_legendas():
        ...
        return r
    passo(5, "Gerando as legendas (rodízio A/B)", gerar_legendas)

    # 6. painéis
    def gerar_paineis():
        print(f"    {dashboard.gerar()}")
        print(f"    {painel.gerar()}")
    passo(6, "Atualizando os painéis", gerar_paineis, obrigatorio=False)
    ...
    def checar_conformidade():
        ...
        return r
    passo(7, "Checklist de conformidade", checar_conformidade, obrigatorio=False)
    ...
    if publicar_redes and _ja_publicado(proximo):
        print(f"\n[8-9] Concurso {proximo} já foi publicado antes. Publicação cancelada.")
        publicar_redes = False

    if publicar_redes:
        passo(8, "Copiando imagens para a pasta pública",
              lambda: publicar.publicar_no_site(proximo), obrigatorio=False)

        def publicar_tudo():
            for rede, funcao in (
                ("Instagram", lambda: publicar.publicar_instagram(proximo, "dia", "carrossel", simular)),
                ("TikTok", lambda: publicar.publicar_tiktok(proximo, "noite", simular)),
            ):
                try:
                    publicar.registrar(proximo, funcao())
                except publicar.PublicacaoError as erro:
                    print(f"    {rede} não publicou: {erro}")
        passo(9, "Publicando nas redes" + (" (SIMULAÇÃO)" if simular else ""),
              publicar_tudo, obrigatorio=False)
    else:
        print("\n[8-9] Publicação desligada")
